...
Each reviewer has a 5-minute timeout per round; if a model hangs or rate-limits, that sub-session is aborted and the reviewer reports a failure (the other two continue). **On failure, the slot is retried with the next free model from the ranked pool** (NIM / other Zen free models), up to 6 replacements per run — so one stuck model doesn't sink the whole review. If *every* reviewer times out, the run ends early with a warning instead of a consolidated review, and you're told to retry later (the free tier is often loaded). The header of the output shows which models were used, how many rounds ran, and total elapsed time.
